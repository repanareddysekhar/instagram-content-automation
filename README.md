...
It runs immediately in mock mode, so you can inspect the full workflow without API keys. Live mode uses OpenAI for structured scripts and optional art, Telegram inline buttons for approval, and Instagram's official publishing API for carousels.
...
- OpenAI Responses API script generation with a strict JSON schema
...
- Optional GPT Image 2 artwork behind deterministic typography
...
- `gpt-5.6-terra` is the default text model because content drafting and structured extraction are balanced, repeatable workloads.
- `reasoning.effort` is explicitly `low` to keep latency and cost controlled.
- `gpt-image-2` is opt-in through `ENABLE_AI_ART=true`; the deterministic renderer always owns typography because generated images can still be inconsistent with exact text placement.

Current OpenAI documentation: [Responses and GPT-5.6 guidance](https://developers.openai.com/api/docs/guides/latest-model?model=gpt-5.6), [image generation](https://developers.openai.com/api/docs/guides/image-generation).
